 ...
 */
	...
	$.jstree.core.prototype = {
		...
		is_parent : function (obj) {
			obj = this.get_node(obj);
			return obj && (obj.state.loaded === false || obj.pages.length > 0);
		},
		/**
		...
		load_node : function (obj, callback) {
			...
			this._load_node(obj, $.proxy(function (status) {
				...
				if(obj.state.loaded && !obj.pages.length && dom && dom.length && !dom.hasClass('jstree-leaf')) {
				...
				func = function (data, undefined) {
					...
					var dat = data.dat,
						...
						parse_flat = function (d, p, ps) {
							...
							if(d && d.pages && d.pages === true) {
								tmp.state.loaded = false;
								tmp.pages = [];
								tmp.children_d = [];
							}
							m[tmp.id] = tmp;
							for(i = 0, j = tmp.pages.length; i < j; i++) {
								c = parse_flat(m[tmp.pages[i]], tmp.id, ps);
								e = m[c];
						...
						parse_nest = function (d, p, ps) {
							...
							if(d && d.pages && d.pages.length) {
								for(i = 0, j = d.pages.length; i < j; i++) {
									c = parse_nest(d.pages[i], tmp.id, ps);
									e = m[c];
									tmp.pages.push(c);
									if(e.children_d.length) {
										tmp.children_d = tmp.children_d.concat(e.children_d);
								...
								tmp.children_d = tmp.children_d.concat(tmp.pages);
							}
							if(d && d.pages && d.pages === true) {
								tmp.state.loaded = false;
								tmp.pages = [];
					...
					if(dat.length && dat[0].id !== undefined && dat[0].parent !== undefined) {
						...
						for(i = 0, j = dat.length; i < j; i++) {
							m[dat[i].parent.toString()].pages.push(dat[i].id.toString());
							// populate parent.children_d
							p.children_d.push(dat[i].id.toString());
						}
						// 3) normalize && populate parents and children_d with recursion
						for(i = 0, j = p.pages.length; i < j; i++) {
							tmp = parse_flat(m[p.pages[i]], par, p.parents.concat());
							dpc.push(tmp);
							if(m[tmp].children_d.length) {
								dpc = dpc.concat(m[tmp].children_d);
							}
		...
		_parse_model_from_html : function (d, p, ps) {
			...
			if(tmp.length) {
				...
				data.children_d = data.children_d.concat(data.pages);
			}
			else {
		...
		_parse_model_from_flat_json : function (d, p, ps) {
			...
			m[tmp.id] = tmp;
			for(i = 0, j = tmp.pages.length; i < j; i++) {
				c = this._parse_model_from_flat_json(m[tmp.pages[i]], tmp.id, ps);
				e = m[c];
				tmp.children_d.push(c);
				if(e.children_d.length) {
					tmp.children_d = tmp.children_d.concat(e.children_d);
		...
		_parse_model_from_json : function (d, p, ps) {
			...
			if(d && d.pages && d.pages.length) {
				for(i = 0, j = d.pages.length; i < j; i++) {
					c = this._parse_model_from_json(d.pages[i], tmp.id, ps);
					e = m[c];
					tmp.pages.push(c);
					if(e.children_d.length) {
						tmp.children_d = tmp.children_d.concat(e.children_d);
		 ...
		 */
		_redraw : function () {
			var nodes = this._model.force_full_redraw ? this._model.data['#'].pages.concat([]) : this._model.changed.concat([]),
		...
		redraw_node : function (node, deep, is_callback) {
			...
			if(!node) {
				...
				if(!is_callback) {
					...
					ind = $.inArray(obj.id, par === null ? m['#'].pages : m[obj.parent].pages);
				}
			}
			else {
				node = $(node);
				...
				if(!deep && obj.pages.length && !node.children('.jstree-children').length) {
					deep = true;
				}
				if(!deep) {
			...
			if(obj.state.loaded && !obj.pages.length) {
				c += ' jstree-leaf';
			}
			else {
			...
			if(obj.parent !== null && m[obj.parent].pages[m[obj.parent].pages.length - 1] === obj.id) {
				c += ' jstree-last';
			}
			node.id = obj.id;
			node.className = c;
			...
			if(deep && obj.pages.length && obj.state.opened && obj.state.loaded) {
				k = d.createElement('UL');
				k.setAttribute('role', 'group');
				k.className = 'jstree-children';
				for(i = 0, j = obj.pages.length; i < j; i++) {
					k.appendChild(this.redraw_node(obj.pages[i], deep, true));
				}
				node.appendChild(k);
			}
			if(old) {
		...
		/**
		 * opens a node, revaling its pages. If the node is not loaded it will be loaded and opened once ready.
		...
		open_node : function (obj, callback, animation) {
			...
			if(!this.is_loaded(obj)) {
				...
				t = this;
				if(d.length) {
					if(obj.pages.length && !this._firstChild(d.children('.jstree-children')[0])) {
		...
		/**
		 * opens all nodes within a node (or the tree), revaling their pages. If the node is not loaded it will be loaded and opened once ready.
			...
			if(!this.settings.core.multiple || (!e.metaKey && !e.ctrlKey && !e.shiftKey) || (e.shiftKey && (!this._data.core.last_clicked || !this.get_parent(obj) || this.get_parent(obj) !== this._data.core.last_clicked.parent ) )) {
				...
				if(e.shiftKey) {
					var o = this.get_node(obj).id,
						...
						p = this.get_node(this._data.core.last_clicked.parent).pages,
						c = false,
						i, j;
					for(i = 0, j = p.length; i < j; i += 1) {
		...
		set_id : function (obj, id) {
			...
			m[obj.parent].pages[$.inArray(obj.id, m[obj.parent].pages)] = id;
			for(i = 0, j = obj.parents.length; i < j; i++) {
				m[obj.parents[i]].children_d[$.inArray(obj.id, m[obj.parents[i]].children_d)] = id;
			}
			// update children (replace current ID with new one in parent and parents)
			for(i = 0, j = obj.pages.length; i < j; i++) {
				m[obj.pages[i]].parent = id;
			}
			for(i = 0, j = obj.children_d.length; i < j; i++) {
		...
		get_text : function (obj) {
			obj = this.get_node(obj);
			return (!obj || obj.id === '#') ? false : obj.name;
		},
		/**
		...
		get_json : function (obj, options, flat) {
			...
			if(!options || !options.no_children) {
				for(i = 0, j = obj.pages.length; i < j; i++) {
					if(options && options.flat) {
						this.get_json(obj.pages[i], options, flat);
					}
					else {
						tmp.pages.push(this.get_json(obj.pages[i], options));
					}
				}
			}
			return options && options.flat ? flat : (obj.id === '#' ? tmp.pages : tmp);
		},
		/**
		...
		create_node : function (par, node, pos, callback, is_loaded) {
			...
			switch(pos) {
				case "before":
					...
					pos = $.inArray(par.id, tmp.pages);
					par = tmp;
					break;
				case "after" :
					tmp = this.get_node(par.parent);
					pos = $.inArray(par.id, tmp.pages) + 1;
					par = tmp;
					break;
				case "inside":
				case "first":
					...
					break;
				case "last":
					pos = par.pages.length;
					break;
				default:
					if(!pos) { pos = 0; }
					break;
			}
			if(pos > par.pages.length) { pos = par.pages.length; }
			...
			node = tmp;
			tmp = [];
			for(i = 0, j = par.pages.length; i < j; i++) {
				tmp[i >= pos ? i+1 : i] = par.pages[i];
			}
			tmp[pos] = node.id;
			par.pages = tmp;
		...
		delete_node : function (obj) {
			...
			if(pos !== -1) {
				par.pages = $.vakata.array_remove(par.pages, pos);
			}
		...
		check : function (chk, obj, par, pos, more) {
			...
			if(chk === "move_node" || chk === "copy_node") {
				if((!more || !more.is_multi) && (obj.id === par.id || $.inArray(obj.id, par.pages) === pos || $.inArray(par.id, obj.children_d) !== -1)) {
		...
		move_node : function (obj, par, pos, callback, is_loaded) {
			...
			old_pos = old_ins && old_ins._id && old_par && old_ins._model.data[old_par] && old_ins._model.data[old_par].pages ? $.inArray(obj.id, old_ins._model.data[old_par].pages) : -1;
			if(is_multi) {
			...
			switch(pos) {
				case "before":
					pos = $.inArray(par.id, new_par.pages);
					break;
				case "after" :
					pos = $.inArray(par.id, new_par.pages) + 1;
					break;
				case "inside":
				case "first":
					pos = 0;
					break;
				case "last":
					pos = new_par.pages.length;
					break;
				default:
					if(!pos) { pos = 0; }
					break;
			}
			if(pos > new_par.pages.length) { pos = new_par.pages.length; }
			...
			if(obj.parent === new_par.id) {
				...
				old_ins._model.data[old_par].pages = $.vakata.array_remove_item(old_ins._model.data[old_par].pages, obj.id);
				...
				dpc = [];
				for(i = 0, j = new_par.pages.length; i < j; i++) {
					dpc[i >= pos ? i+1 : i] = new_par.pages[i];
				}
				dpc[pos] = obj.id;
				new_par.pages = dpc;
		...
		copy_node : function (obj, par, pos, callback, is_loaded) {
			 ...
			 * @param {jsTree} new_instance the instance of the new parent
			 */
			this.trigger('copy_node', { "node" : tmp, "original" : obj, "parent" : new_par.id, "position" : pos, "old_parent" : old_par, "old_position" : old_ins && old_ins._id && old_par && old_ins._model.data[old_par] && old_ins._model.data[old_par].pages ? $.inArray(obj.id, old_ins._model.data[old_par].pages) : -1,'is_multi' : (old_ins && old_ins._id && old_ins._id !== this._id), 'is_foreign' : (!old_ins || !old_ins._id), 'old_instance' : old_ins, 'new_instance' : this });
			return tmp.id;
		},
		/**
					...
					.on('model.jstree', $.proxy(function (e, data) {
							...
							if(s.indexOf('up') !== -1) {
								...
								for(i = 0, j = p.children_d.length; i < j; i++) {
									if(!m[p.children_d[i]].pages.length) {
					...
					.on('select_node.jstree', $.proxy(function (e, data) {
							...
							if(s.indexOf('up') !== -1) {
								while(par && par.id !== '#') {
									c = 0;
									for(i = 0, j = par.pages.length; i < j; i++) {
										c += m[par.pages[i]].state.selected;
									}
									if(c === j) {
							...
							$.each(o, function (j, v) {
								switch(v) {
									...
									case 'i':
										...
										t = off.top + h / 2 + 1;
										p = tm.id;
										i = ip === 'first' ? 0 : (ip === 'last' ? tm.pages.length : Math.min(ip, tm.pages.length));
										break;
									case 'a':
										l = off.left - 6;
										t = off.top + h;
								...
								*/
								...
								for(t1 = 0, t2 = data.data.nodes.length; t1 < t2; t1++) {
									...
									if(op === "move_node" && v === 'a' && (data.data.origin && data.data.origin === ins) && p === ins.get_parent(data.data.nodes[t1])) {
										pr = ins.get_node(p);
										if(ps > $.inArray(data.data.nodes[t1], pr.pages)) {
											ps -= 1;
										}
									}
	...
	$.jstree.plugins.search = function (options, parent) {
		...
		this.search = function (str, skip_async) {
			...
			$.each(this._model.data, function (i, v) {
				if(v.name && ( (s.search_callback && s.search_callback.call(this, str, v)) || (!s.search_callback && f.search(v.name).isMatch) ) && (!s.search_leaves_only || (v.state.loaded && v.pages.length === 0)) ) {
					r.push(i);
	...
	$.jstree.plugins.sort = function (options, parent) {
		...
		this.sort = function (obj, deep) {
			...
			if(obj && obj.pages && obj.pages.length) {
				obj.pages.sort($.proxy(this.settings.sort, this));
				if(deep) {
					for(i = 0, j = obj.children_d.length; i < j; i++) {
 ...
 */
	 ...
	 * * `max_depth` the maximum number of nesting this node type can have. A value of `1` would mean that the node can have children, but no grandpages. Do not specify or set to `-1` for unlimited.
	 * * `valid_children` an array of node type strings, that nodes of this type can have as pages. Do not specify or set to `-1` for no limits.
	...
	$.jstree.plugins.types = function (options, parent) {
		...
		this._delete_ids = function (tmp) {
			...
			if(tmp.pages && $.isArray(tmp.pages)) {
				tmp.pages = this._delete_ids(tmp.pages);
			}
			return tmp;
		};
		this.check = function (chk, obj, par, pos, more) {
			...
			switch(chk) {
				...
				case "move_node":
				case "copy_node":
					if(chk !== 'move_node' || $.inArray(obj.id, par.pages) === -1) {
						tmp = this.get_rules(par);
						if(tmp.max_children !== undefined && tmp.max_children !== -1 && tmp.max_children === par.pages.length) {
 ...
 */
	...
	$.jstree.plugins.unique = function (options, parent) {
		this.check = function (chk, obj, par, pos, more) {
			...
			obj = obj && obj.id ? obj : this.get_node(obj);
			par = par && par.id ? par : this.get_node(par);
			if(!par || !par.pages) { return true; }
			var n = chk === "rename_node" ? pos : obj.name,
				c = [],
				s = this.settings.unique.case_sensitive,
				m = this._model.data, i, j;
			for(i = 0, j = par.pages.length; i < j; i++) {
				c.push(s ? m[par.pages[i]].name : m[par.pages[i]].name.toLowerCase());
			}
			...
			switch(chk) {
				case "delete_node":
					return true;
				case "rename_node":
					i = ($.inArray(n, c) === -1 || (obj.name && obj.name === n));
					if(!i) {
		...
		this.create_node = function (par, node, pos, callback, is_loaded) {
			if(!node || node.name === undefined) {
				...
				dpc = [];
				for(i = 0, j = par.pages.length; i < j; i++) {
					dpc.push(s ? m[par.pages[i]].name : m[par.pages[i]].name.toLowerCase());
				}
				i = 1;
